...
If `state.version != "6.0"` OR state file is missing/empty: migrate/initialize using this exact Python snippet (do NOT use `cat file | python3 > file` — that truncates the file before reading it):
...
stats.setdefault("agentRoutingStats", {"frontend": 0, "backend": 0, "devops": 0, "ml-engineer": 0, "test-engineer": 0, "general-purpose": 0, "llmOverrides": 0})
stats.setdefault("reviewStats", {"totalFindings": 0, "filteredByConfidence": 0, "avgConfidence": 0})
state["version"] = "6.0"
